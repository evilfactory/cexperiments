...
{
	// problema: temos um buraco na array agora, e o proximo paciente que for inserido, sera colocado nele, que não é oq queremos, queremos que ele seja inserido no final
...
Patient prompt_find_patient(Patient elderly[], Patient adults[], int size)
{
	...
	for (int i = 0; i < size; i++)
	{
		if (elderly[i].Valid && strcmp(contact, elderly[i].Contact))
		{
			return elderly[i];
		}
		if (adults[i].Valid && strcmp(contact, adults[i].Contact))
		{
			return adults[i];
		}
	}
	...
	while (1)
	{
		...
		switch (op)
		{
			...
			case 'l':
				...
				break;
			case 'v':
				Patient patient = prompt_find_patient(elderly, adults, 100);
				if (patient.Valid)
				{
					print_patient(patient);
				}
